...
import { getCallDetails, getCallHistory, getRecStream } from '../../common/api';
...
const History: React.FC<HistoryProps> = () => {
    ...
    // Simplified audio fetching - just set the URL directly, browser handles range requests
    useEffect(() => {
        ...
        if (sideBarData?.call_id) {
            try {
                setAudioLoading(true);
                setAudioError(null);
                
                // Get the direct URL - browser will handle range requests automatically
                const audioUrl = getRecStream(sideBarData.call_id);
    ...
    const open = async (rowData: any) => {
        ...
        setSideBarData({
            call_id: conversationId,  // Use call_id instead of recording_api
                        ...
                        <div className='audio-section'>
                            ...
                            {audioSrc && !audioError && (
                                <audio 
                                    ...
                                    style={{width: '100%'}}
                                    onError={() => {
                                        setAudioError("Failed to load audio. The recording may not be available.");
                                    }}
                                ></audio>
                            )}
                        </div>
